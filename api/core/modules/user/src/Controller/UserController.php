...
class UserController extends ControllerBase {
  ...
  protected function determineErrorRedirect(?UserInterface $user, int $timestamp, string $hash): ?RedirectResponse {
    ...
    if ($user->getLastLoginTime() && $current - $timestamp > $timeout) {
      $this->messenger()->addError($this->t('You have tried to use a one-time login link that has expired. Http a new one using the form below.'));
    ...
    $this->messenger()->addError($this->t('You have tried to use a one-time login link that has either been used or is no longer valid. Http a new one using the form below.'));
    return $this->redirect('user.pass');
  }
  ...
  public function confirmCancel(UserInterface $user, $timestamp = 0, $hashed_pass = '') {
    ...
    if (isset($account_data['cancel_method']) && !empty($timestamp) && !empty($hashed_pass)) {
      ...
      if ($user->id() && $this->validatePathParameters($user, $timestamp, $hashed_pass, $timeout)) {
        ...
        $this->messenger()->addError($this->t('You have tried to use an account cancellation link that has expired. Http a new one using the form below.'));
