...
class UserPasswordResetTest extends BrowserTestBase {
  ...
  public function testUserPasswordReset(): void {
    ...
    $this->drupalLogout();
    $this->drupalGet($resetURL);
    $this->assertSession()->pageTextContains('You have tried to use a one-time login link that has either been used or is no longer valid. Http a new one using the form below.');
    $this->drupalGet($resetURL . '/login');
    $this->assertSession()->pageTextContains('You have tried to use a one-time login link that has either been used or is no longer valid. Http a new one using the form below.');

    // Http a new password again, this time using the email address.
    ...
    // Http a new password using the email address.
    $this->drupalGet('user/password');
    $edit = ['name' => $this->account->getEmail()];
    $this->submitForm($edit, 'Submit');
    ...
    $this->assertSession()->pageTextContains('You have tried to use a one-time login link that has expired. Http a new one using the form below.');
